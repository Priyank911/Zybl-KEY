...
const BiometricVerification = () => {
  ...
  useEffect(() => {
    if (currentStep === 0 && !isVerifying && !verificationComplete) {
      let stream = null;
      ...
      const startVideo = async () => {
        try {
          ...
          // For demo purposes, continue without actual camera
          setTimeout(() => startFakeVerification(), 1000);
        }
      };
  ...
  }, [currentStep, isVerifying, verificationComplete]);

  // Generate random data for verification metrics
  const generateRandomData = (step) => {
    const metrics = {};
    
    switch (step) {
      case 0: // Face verification
        metrics.faceMatch = Math.floor(90 + Math.random() * 10);
        metrics.liveness = Math.random() > 0.1 ? 'Verified' : 'Warning';
        metrics.uniqueId = `0x${Math.random().toString(16).substring(2, 8)}`;
        break;
  ...
  // Fake verification for cases where camera isn't available
  const startFakeVerification = () => {
    setIsVerifying(true);
    
    // Draw a fake face outline on canvas
    if (canvasRef.current) {
      ...
      for (let i = 0; i < 8; i++) {
        const y = centerY - boxSize/2 + (i * boxSize/7);
        ctx.beginPath();
        ctx.moveTo(centerX - boxSize/2, y);
        ctx.lineTo(centerX + boxSize/2, y);
        ctx.stroke();
      }
      ...
      const points = [
        [centerX - 40, centerY - 30], // left eye
        [centerX + 40, centerY - 30], // right eye
        [centerX, centerY], // nose
        [centerX - 30, centerY + 40], // left mouth
        [centerX + 30, centerY + 40], // right mouth
        [centerX - 60, centerY - 50], // left edge
        [centerX + 60, centerY - 50], // right edge
        [centerX - 70, centerY + 20], // left jaw
        [centerX + 70, centerY + 20], // right jaw
        [centerX, centerY + 60] // chin
      ];
      ...
      ctx.strokeStyle = 'rgba(0, 240, 120, 0.5)';
      ctx.beginPath();
  ...
  return (
    <div className="biometric-container">
      ...
      <div className="biometric-card">
        {verificationComplete ? (          <div className="verification-complete">
                  ...
                  <div className="face-verification-content">
                    <div className="face-scan-container">
                      <video 
                        ...
                        muted 
                        playsInline
                        className={isVerifying ? 'scanning' : ''}
                      ></video>
                      <canvas 
                        ref={canvasRef}
                        width="320"
